...
//    }
    ...
    public void testRestartTimer() {
        ...
        LocoNetSystemConnectionMemo memo = slotmanager.getSystemConnectionMemo();
        Assert.assertNotNull("check memo", memo);
        jmri.InstanceManager.setDefault(jmri.PowerManager.class,new jmri.jmrix.loconet.LnPowerManager(memo));
        JUnitUtil.waitFor(()->{return (lnis.outbound.size()==1);},"Power manager has sent its slot query");

        int sentCount = lnis.outbound.size();
        ...
        for (int i = 0; i < commandStations.length; ++i) {
            ...
            Assert.assertEquals("check no new messages sent yet for "+c.name,
                    sentCount, lnis.outbound.size());
            slotmanager.setCommandStationType(c);
            Assert.assertEquals(0, slotmanager.progState);
            Assert.assertEquals(true, slotmanager.mServiceMode);
            slotmanager.restartEndOfProgrammingTimer();
            JUnitUtil.waitFor(11000);
            ...
            switch (c) {
                ...
                case COMMAND_STATION_DB150:
                    sentCount++;
                    Assert.assertEquals("check got a new message sent yet for "+c.name,
                            sentCount, lnis.outbound.size());
                    Assert.assertEquals("Check opcode of sent message for "+c.name,
                            LnConstants.OPC_GPON,
                            lnis.outbound.get(lnis.outbound.size()-1).getOpCode());
                    break;
                default:
                    Assert.fail("command station named " + c.name +
                            " has no programmed test for forced track power on " +
                            "after programming operation");
            }
        }
    }
    ...
    public void setUp() {
        ...
        slotmanager.setSystemConnectionMemo(memo);
